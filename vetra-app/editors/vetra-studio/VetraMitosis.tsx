...
const QUADRANTS = Object.keys(SIGN) as Quadrant[];

const GUTTER = 9.93;
const off = (gap: number) => (gap - GUTTER) / 2;

type Frame = { gapX: number; gapY: number };
...
function poseFromFrame(f: Frame): Pose {
  const ox = off(f.gapX);
  const oy = off(f.gapY);
  const p = {} as Pose;
  for (const k of QUADRANTS) p[k] = [SIGN[k].x * ox, SIGN[k].y * oy];
  return p;
}

function easeOutBack(t: number, k = 1.6) {
  const c1 = k;
  const c3 = c1 + 1;
...
export default function VetraMitosis({ size = 120, color = '#04C161', active = true }: VetraMitosisProps) {
  ...
  useEffect(() => {
    ...
    let clock = 0;
    let logoResumeAt = 0;
    let logoHoldSeg: Segment | null = null;
    ...
    const cur = {} as Pose;
    for (const k of QUADRANTS) cur[k] = [LOGO_POSE[k][0], LOGO_POSE[k][1]];
    let curScale = 1;
    ...
    // When active turns off mid-loop, keep playing to the logo rest pose rather
    // than snapping there, so the animation finishes naturally before settling.
    let finishing = false;

    function tick(ts: number) {
      const isActive = activeRef.current;

      if (isActive && (!wasActive || resume)) {
        // From a settled idle, resync the loop to the logo rest; from a play-out
        // (finishing), continue seamlessly from where it is.
        if (!finishing) t0 = ts - logoResumeAt;
        resume = false;
        finishing = false;
      ...
      if (isActive || finishing) {
        ...
        for (const seg of timeline) {
          if (time >= seg.t0 && time < seg.t1) {
            ...
            // Reached the logo rest while finishing — hand off to the idle settle.
            if (finishing && seg === logoHoldSeg) finishing = false;
            break;
          }
        }
      } else {
      ...
      for (const k of QUADRANTS) apply(k, cur[k][0], cur[k][1], curScale);
      wasActive = isActive;
