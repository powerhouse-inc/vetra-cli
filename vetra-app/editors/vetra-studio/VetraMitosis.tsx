...
type FrameKey = "blob" | "twoV" | "logo" | "twoH";
type Pose = Record<QuadKey, [number, number]>;

const PATHS: Record<QuadKey, string> = {
...
const SIGN: Record<QuadKey, { x: number; y: number }> = {
  tl: { x: -1, y: -1 },
  tr: { x: 1, y: -1 },
  bl: { x: -1, y: 1 },
  br: { x: 1, y: 1 },
...
const GUTTER = 9.93;
const off = (gap: number) => (gap - GUTTER) / 2;

const FRAMES: Record<FrameKey, { gapX: number; gapY: number }> = {
...
const SEQ: { frame: FrameKey; hold: number; trans: number }[] = [
...
function poseFromFrame(f: { gapX: number; gapY: number }): Pose {
  const ox = off(f.gapX);
  const oy = off(f.gapY);
  const p = {} as Pose;
  for (const k in SIGN) p[k as QuadKey] = [SIGN[k as QuadKey].x * ox, SIGN[k as QuadKey].y * oy];
  return p;
}

function easeOutBack(t: number, k = 1.6): number {
  const c1 = k;
  const c3 = c1 + 1;
...
export default function VetraMitosis({
  ...
  useEffect(() => {
    const apply = (k: string, tx: number, ty: number, sc: number) => {
      const el = refs.current[k];
      if (el) el.style.transform = `translate(${tx}px,${ty}px) rotate(180deg) scale(${sc})`;
    };

    if (!active) {
      // Snap to static logo pose and stop — no loop needed.
      for (const k in SIGN) apply(k, LOGO_POSE[k as QuadKey][0], LOGO_POSE[k as QuadKey][1], 1);
      return;
    }
    ...
    const cur: Pose = {} as Pose;
    for (const k in SIGN) cur[k as QuadKey] = [LOGO_POSE[k as QuadKey][0], LOGO_POSE[k as QuadKey][1]];
    let curScale = 1;
    let t0: number | null = null;

    function tick(ts: number) {
      if (t0 === null) t0 = ts - logoResumeAt;
      ...
      for (const seg of timeline) {
        if (time >= seg.t0 && time < seg.t1) {
          if (seg.type === "hold") {
            ...
            for (const k in SIGN) {
              cur[k as QuadKey][0] = seg.pose[k as QuadKey][0];
              cur[k as QuadKey][1] = seg.pose[k as QuadKey][1];
            }
          } else {
            const e = easeOutBack((time - seg.t0) / (seg.t1 - seg.t0));
            curScale = 1;
            for (const k in SIGN) {
              cur[k as QuadKey][0] = lerp(seg.from[k as QuadKey][0], seg.to[k as QuadKey][0], e);
              cur[k as QuadKey][1] = lerp(seg.from[k as QuadKey][1], seg.to[k as QuadKey][1], e);
            }
          }
          break;
        }
      ...
      for (const k in SIGN) apply(k, cur[k as QuadKey][0], cur[k as QuadKey][1], curScale);
  ...
  return (
    ...
    >
      {(Object.keys(PATHS) as QuadKey[]).map((k) => (
        <g
          key={k}
