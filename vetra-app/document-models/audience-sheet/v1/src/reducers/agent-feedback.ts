import type { AudienceSheetAgentFeedbackOperations } from "document-models/audience-sheet/v1";
import {
...
export const audienceSheetAgentFeedbackOperations: AudienceSheetAgentFeedbackOperations =
  {
    ...
    addSuggestionOperation(state, action) {
      if (
        state.agentFeedback.suggestions.some((s) => s.id === action.input.id)
      ) {
