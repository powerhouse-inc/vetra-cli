...
#
# Speed: Dockerfile.prodclose adds a BuildKit cache mount for the pnpm store, so
# the third-party download closure (npmjs-proxied via the edge) is reused across
# runs instead of re-fetched. First run is cold; repeats are minutes.
#
...
# a fresh checkout has no pre-built dist/connect. Build it so the published
...
( cd "${VETRA_ROOT}/vetra-app" && pnpm exec ph-cli connect build \
    --outDir dist/connect --default-drives-url http://__ph_drive_url__ )
